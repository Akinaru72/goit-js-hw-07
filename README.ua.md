...
Перед виконанням домашньої роботи обов’язково ознайомся із цим відео: [https://www.youtube.com/watch?v=bzReZsR_yBY](https://www.youtube.com/watch?v=bzReZsR_yBY)

Після внесення змін у свій репозиторій почекай, будь ласка, 5 хвилин перед відправкою роботи на перевірку. GitHub потребує часу для оновлення версії.

## Перевір себе — наразі ти маєш уявлення про:

- що таке об’єктна модель документа та як будується ієрархія DOM-елементів
- основні властивості та атрибути DOM-елементів
- як маніпулювати DOM-деревом, створюючи та видаляючи елементи
- сутність та функціонал подій у браузері
- обробку подій із використанням методів `addEventListener()` та `removeEventListener()`
- як застосовувати події елементів форм

Прийшов час закріпити ці знання на практиці!
...
3. Завдання виконані у точній відповідності до ТЗ (забороняється змінювати вихідний HTML)
4. У консолі не повинно бути помилок чи попереджень під час відкриття живої сторінки
5. Переконайся, що код відформатований за допомогою **Prettier**
...
Завантажуй стартові файли з готовою розміткою та підключеними скриптами для кожного завдання. Скопіюй їх у свій проєкт.

> Важливо: стартові файли знаходяться у папці `src`.  
> Для GitHub Pages файл `index.html` повинен бути в корені проєкту, тому переноси **тільки вміст папки `src`**, сама папка `src` не потрібна.
